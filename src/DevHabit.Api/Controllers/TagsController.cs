        ...
        UserContext userContext) : ControllerBase
{
    ...
    [HttpGet("{id}")]
    public async Task<IActionResult> GetTag(string id, TagParameters tagParameters, CancellationToken cancellationToken)
    {
        string? userId = await _userContext.GetUserIdAsync(cancellationToken);
            ...
            CancellationToken cancellationToken)
    {
        ...
        bool tagExists = await _dbContext.Tags
            .AnyAsync(x => EF.Functions.ILike(x.Name, createTagDto.Name) && x.UserId == userId, cancellationToken);
    ...
    [HttpPut("{id}")]
    public async Task<IActionResult> UpdateTag(string id, UpdateTagDto updateTagDto, CancellationToken cancellationToken)
    {
        string? userId = await _userContext.GetUserIdAsync(cancellationToken);
        ...
        Tag? tag = await _dbContext.Tags.FirstOrDefaultAsync(x => x.Id == id && x.UserId == userId, cancellationToken);
        ...
        bool tagWithNameExists = await _dbContext.Tags
            .AnyAsync(x => x.Id != id && EF.Functions.ILike(x.Name, updateTagDto.Name) && x.UserId == userId, cancellationToken);
